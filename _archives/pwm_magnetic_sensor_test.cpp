...
#include <SimpleFOC.h>
 ...
 #define ANALOG_PIN 14  // GPIO36 (ADC1_CH0) de l'ESP32

void setup() {
  pinMode(PWM_PIN, INPUT);
  Serial.begin(115200);  // Initialisation du port série
  Serial.println("Lecture du port analogique 36...");
}

void loop() {
  uint32_t highTime = pulseIn(PWM_PIN, HIGH);  // Durée du signal HIGH en µs
  uint32_t lowTime = pulseIn(PWM_PIN, LOW);    // Durée du signal LOW en µs
  uint32_t period = highTime + lowTime;        // Période complète du signal

  if (period > 0) {  // Éviter la division par zéro
    float dutyCycle = (float)highTime / period;  // Rapport cyclique
    float angle = dutyCycle * 360.0;             // Conversion en degrés

    Serial.print("Angle: ");
    Serial.print(angle);
    Serial.println("°");
  } else {
    Serial.println("Aucun signal détecté !");
  }
